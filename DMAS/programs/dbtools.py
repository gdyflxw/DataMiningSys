...
class dbtools:
    ...
    def getconstr(self,dbtype='mssql',conp=None):
        if conp is None:constr=''
        if dbtype=='mssql':
            constr="mssql+pymssql://%s:%s@%s/%s"%(conp[0],conp[1],conp[2],conp[3])
        elif dbtype=='postgresql':
            constr="postgresql://%s:%s@%s/%s"%(conp[0],conp[1],conp[2],conp[3])
            sql="set search_path to %s;"%conp[4]+sql
        elif dbtype=='mysql':
            constr="mysql+pymysql://%s:%s@%s/%s?charset=utf8"%(conp[0],conp[1],conp[2],conp[3])
        elif dbtype=='oracle':
            constr="oracle+cx_oracle://%s:%s@%s/?service_name=%s"%(conp[0],conp[1],conp[2],conp[3])
        elif dbtype=='sqlite':
            constr="sqlite:///%s"%conp[2]
        return constr
